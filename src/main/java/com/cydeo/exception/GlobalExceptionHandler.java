...
@RestControllerAdvice // AOP whenever exception is thrown this class will be executed
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseWrapper> serviceException(TicketingProjectException se){
        // modifying the json output in swagger if exception occurs show this output
    ...
    @ExceptionHandler(AccessDeniedException.class) // if exception belongs to AccessDeniedException.class use this method
    ...
    public ResponseEntity<ResponseWrapper> genericException(Throwable e, HandlerMethod handlerMethod){
        ...
        // if we have annotation our message is built
        if (defaultMessage.isPresent() && !ObjectUtils.isEmpty(defaultMessage.get().getMessage())) { //defaultMessage our annotation defaultexceptionmessage
        ...
        return new ResponseEntity<>(ResponseWrapper.builder().success(false).message("Action failed: An error occured!").code(HttpStatus.INTERNAL_SERVER_ERROR.value()).build(),HttpStatus.INTERNAL_SERVER_ERROR);
    }
